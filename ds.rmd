...
## Project Subject
Almost everyone asket themselves at least once the following question: *What influences love at first site?*. In order to get some insights I downloaded a Speed dateing dataset from kaggle. 

The dataset was created by Columbia Business School professors for their paper *Gender Differences in Mate Selection: Evidence From a Speed Dating Experiment*.

The data in the dataset was collected from a participants of multiple experimental speed datingevents. The speed dating experiment setup was the following:

* 4 minute conversation with a participant of opposite sex
* Complete a questionare about the "first date"
...
After deleting majority of the columns we have 17 most importatnt variables remaining, which we will use for prediction. 

```{r vars}
pander(colnames(sdd))
```

See the first few rows of the dataset in the following table:

```{r head}
sdd <- na.omit(sdd)
```

As a last step I will omit rows with NA values and this will left us with 6838 observations and a clean dataset.
...
For persons where the it is more important that the partner is of same religious background it is more importat that they are of same racial background
...
More intelligent persons are more attaractive on average
...
For modelling I will create Train and Test datasets. The datasets will be created with randomly choosing observations in the following breakdown:

* Train dataset - 50% of data, used for training the models
* Test dataset - 50% of data, used for model evaluation
...
* K-nearest neighbours - where K=10,15,30
* Decision tree
...
#### DT1
First decision tree calculated with dec_o variable. This variable shows the decision of the partner so it is logical that it will be na important precitor 
```{r dt, warning=FALSE}
...
Among KNN the best performing model is where K = 30. It is the modell with highest complexity amongst KNN models tested. Computing time for all of the models was pretty fast.
...
The best performing RF model was the one with 500 trees and maximal depth of 15 (RF2). This was the middle comlexity model and it had the middle computation time amongst the random forest models. We can see from the AUC that the RF3 model is already overfitted a bit compated to RF2. The computation time of all RF models was more than in case of decision tree and KNN models.
...
The best performing GBM model was the one with 500 trees and maximal depth of 15 (GBM2). The complexity and computation time was growing from first model to third. The third model has weaker performance from the second so it is overfitted a bit. The GBM models had the highest computation time from all of the models.

## Model selection

The best peroforming model from the models above was the RF2 random forest model with 500 trees and 15 depth. 
